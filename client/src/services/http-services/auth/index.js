...
export const getUserInfo = async () => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API_URL}/auth/get-user-info`,
      {
...
export const userExist = async (values) => {
  ...
  try {
    const { data } = await axios.post(`${process.env.REACT_APP_API_URL}/auth/user-exist`, {
      ...values,
    });
...
export const getAllCustomers = async ({ values, cbSuccess, cbFailure }) => {
  try {
    const { data, status } = await axios.get(`${process.env.REACT_APP_API_URL}/customers`);
